...
tools/check-release-drift.py \
  --releases-file tools/testdata/releases-2026-08-20.json \
...
python3 tools/gen-supported.py \
  --releases-file tools/testdata/releases-2026-08-20.json --check
```
...
with the published split code patch. The current repository checks nine split
forms twice, once for the code half and once for the font half.
...
returns nine font failures with nine code passes, exit 1. A deliberately divergent
...
current repository checks nine kernels.
...
A data patch that rejects exits 1. Setting the `font_cjk_32x32` descriptor's
`.height` to 31 in `v6.x/cjktty-code-6.18.patch` returns `cjk32 patch
application: 8 passed, 1 failed`, exit 1, while the descriptor-order check this
replaced returned exit 0 for the same input.
